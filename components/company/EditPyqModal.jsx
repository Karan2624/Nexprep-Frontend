...
import { updateCompanyPyq } from "@/lib/companyPyqApi";

export default function EditPyqModal({
  pyq,
  onClose,
  onSuccess,
}) {
  ...
  const [formData, setFormData] = useState({
    ...
    question: pyq.description,
    topics: pyq.tags?.join(", ") || "",

    sampleInput: pyq.sampleTestCase?.[0]?.input || "",
    sampleOutput: pyq.sampleTestCase?.[0]?.output || "",
    explanation: pyq.sampleTestCase?.[0]?.explanation || "",
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await updateCompanyPyq(pyq._id, {
      ...
      console.error(err);
      alert(
        err?.response?.data?.message || "Failed to update PYQ."
      );
    } finally {
      setLoading(false);
    }
