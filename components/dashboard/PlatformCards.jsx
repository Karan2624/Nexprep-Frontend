      ...
      <div className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className={`rounded-xl p-5 flex items-center justify-between shadow-md transition-all duration-300 group ${isLcLinked ? 'bg-gradient-to-br from-[#FFA116] to-[#E58F0D]' : 'bg-slate-100 border border-slate-200'}`}>
          <div className="flex items-start sm:items-center gap-4">
            <div className={`w-12 h-12 rounded-lg flex items-center justify-center shrink-0 shadow-sm ${isLcLinked ? 'bg-white text-[#FFA116]' : 'bg-slate-300 text-slate-500'}`}>
              <LeetCodeIcon />
            </div>
            <div>
              ...
              {isLcLinked && (
                <div className="text-[11px] font-medium text-white/90 flex flex-wrap gap-x-2 gap-y-0.5">
                  <span>Rating: {lcRating}</span> <span className="opacity-50 hidden sm:inline">|</span> <span>Rank: {lcRank.toLocaleString()}</span> <span className="opacity-50 hidden sm:inline">|</span> <span>Solved: {lcSolved}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className={`rounded-xl p-5 flex items-center justify-between shadow-md transition-all duration-300 group ${isCfLinked ? 'bg-gradient-to-br from-[#1F8ACB] to-[#1975AE]' : 'bg-slate-100 border border-slate-200'}`}>
          <div className="flex items-start sm:items-center gap-4">
            <div className={`w-12 h-12 rounded-lg flex items-center justify-center shrink-0 shadow-sm ${isCfLinked ? 'bg-white text-[#1F8ACB]' : 'bg-slate-300 text-slate-500'}`}>
              <CodeforcesIcon />
            </div>
            <div>
              ...
              {isCfLinked && (
                <div className="text-[11px] font-medium text-white/90 flex flex-wrap gap-x-2 gap-y-0.5">
                  <span>Max Rating: {cfRating}</span> <span className="opacity-50 hidden sm:inline">|</span> <span>Solved: {cfSolved}</span>
                </div>
              )}
            </div>
          </div>
